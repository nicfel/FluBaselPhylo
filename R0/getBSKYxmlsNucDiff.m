...
clear
system('rm -r bdskynucdiff/xmls');
system('mkdir bdskynucdiff/xmls');
...
for subset = 1 : 10
    ...
    while ~feof(f)
        ...
        if length(members)>0
    %     if length(members)==2 && count<1000
    %     if count < 20
    ...
    figure()
    bar(cl_size_for_plotting.*[1:length(cl_size_for_plotting)]./sum(cl_size_for_plotting.*[1:length(cl_size_for_plotting)]))

    % bar(cl_size_for_plotting)
    ...
    location = cell(0,0);
    f = fopen(['bdskynucdiff/bdsky_subs' num2str(subset) '.xml'],'w');
    ...
    fprintf(f,'\t\t\t\t\t<Normal id="Uniforfsdm.4" name="x0Prior" mean="-0.6931" sigma="0.1"/>\n');
    fprintf(f,'\t\t\t\t</distribution>\n');
    ...
    fprintf(f,'\t<logger id="tracelog.clusters" fileName="clusters_subs%d.log" logEvery="50000" model="@posterior" sanitiseHeaders="true" sort="smart">\n', subset);
    ...
    fprintf(f,'\t<logger id="tracelog.origin" fileName="heights_subs%d.log" logEvery="50000" model="@posterior" sanitiseHeaders="true" sort="smart">\n', subset);
    ...
    fprintf(f,'\t<logger id="tracelog.bdsky" fileName="bdsky_subs%d.log" logEvery="50000" model="@posterior" sanitiseHeaders="true" sort="smart">\n', subset);
    ...
    %% multiply the xmls while also building subsets
    nr_reps = 5;

    for r = 0:nr_reps-1
        f = fopen(['bdskynucdiff/bdsky_subs' num2str(subset) '.xml']);
        g = fopen(sprintf(['bdskynucdiff/xmls/bdsky_subs' num2str(subset) '_rep%d.xml'],r), 'w');
        while ~feof(f)
            line = fgets(f);
            if ~isempty(strfind(line, 'clusters.log'))
                fprintf(g, strrep(line, 'clusters.log', ['clusters_rep' num2str(r) '.log']));
            elseif ~isempty(strfind(line, 'origin.log'))
                fprintf(g, strrep(line, 'origin.log', ['origin_rep' num2str(r) '.log']));
            elseif ~isempty(strfind(line, 'bdsky.log'))
                fprintf(g, strrep(line, 'bdsky.log', ['bdsky_rep' num2str(r) '.log']));
...
% %% build subsets of the clusters for sensitivity analysis
% fclose('all')
% 
% xmls = dir('bdskynucdiff/*.xml');
% 
% for i = 1 : length(xmls)
%     for r = 0 : nr_reps-1
% 
%         f = fopen(['bdskynucdiff/' xmls(i).name]);
%         g = fopen(['bdskynucdiff/xmls/' strrep(xmls(i).name, '.xml', sprintf('_rep%d.xml', r))], 'w');
% 
% 
%         while ~feof(f)
%             line = fgets(f);
