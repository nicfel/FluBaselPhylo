...
def save_as_nexus(tree, fname, metric="div"):
    def format_string_attr(node, key):
        if key!="clade_name" and key!="clade_annotation":
    ...
    def stringify_node(node, prev_div, terminal):
        ...
        print type(attrs_to_write)
        if "clade_name" in attrs_to_write:
            attrs_to_write.remove("clade_name")
        if "clade_annotation" in attrs_to_write:
            attrs_to_write.remove("clade_annotation")
