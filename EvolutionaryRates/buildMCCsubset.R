...
trees <- list.files(path="./subset/out/", pattern="rep0.*\\.trees$", full.names = TRUE)

system("rm -r subset/mcc")
system("mkdir subset/mcc")
system("rm -r subset/combined")
system("mkdir subset/combined")
# run log combiner
for (i in seq(1,length(trees))){
  in_command <- " -trees -burnin 1000000"
  for (j in seq(0,9)){
...
logs <- list.files(path="./subset/out/", pattern="rep0.*\\.log$", full.names = TRUE)

# run log combiner
for (i in seq(1,length(logs))){
  system("rm tmp.trees")
  in_command <- " -burnin 1000000"
  for (j in seq(0,9)){
